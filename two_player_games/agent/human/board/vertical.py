...
class VerticalBoardHuman(BoardHuman[VerticalBoardAction]):
    def select_action(
        self,
        model: VerticalBoard,
    ) -> VerticalBoardAction | None:
        ...
        while self.view.human_action is None:
            ...
            if self.view.human_action is not None:
                # HACK This is a hack to allow for the vertical board. We need
                # a more general way to handle this.
                self.view.human_action = self.view.human_action[1]
                if self.view.human_action not in model.possible_actions:
                    self.view.human_action = None
